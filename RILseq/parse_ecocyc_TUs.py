"""
Parse the genes.dat file to generate data on coverage of the genome and genes
names.
"""
...
def read_REP_table(ec_dir='/home/users/assafp/Database/EcoCyc/current/data',
                   rep_file='REP_elements_table_from_SmartTrables.txt'):
    """
    ...
    Arguments:
    - `ec_dir`: The EcoCyc directory
    - `rep_file`: The table files, coordinates are in columns 3,4,6,7
    """
    rep_pos = {}
    for line in csv.reader(open("%s/%s"%(ec_dir, rep_file)), delimiter='\t'):
...
def position_to_gene(
    ...
    for gname, gpos in uid_pos.items():
        if gname in sRNAs_list:
            continue
        # If no IGT or no promoter or terminator is assigned to any of the IGT
